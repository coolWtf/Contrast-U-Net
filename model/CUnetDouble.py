...
class CUnetDouble(nn.Module):
    def __init__(self,
                 in_channels: int = 1,
                 num_classes: int = 2,
                 bilinear: bool = True,
                 base_c: int = 32):
        super(CUnetDouble, self).__init__()
        ...
        self.con2 = nn.Conv2d(base_c*4*2, base_c*4, kernel_size=3, padding=1, bias=False)

        self.down3 = Down(base_c * 4, base_c * 8)
        self.cvo3 = Contrast(base_c * 4, base_c * 8)
        self.con3 = nn.Conv2d(base_c*8*2, base_c*8, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        x3 = self.down2(x2)
        xc3 = torch.cat((x3, self.cvo2(x2)), dim=1)
        x3 = self.con2(xc3)

        x4 = self.down3(x3)
        xxx = self.cvo3(x3)
        xc4 = torch.cat((x4, self.cvo3(x3)), dim=1)
        x4 = self.con3(xc4)
